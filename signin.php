        ...
        if(!preg_match("/^[a-zA-Z\s]+$/",$fname)){
            ...
            if(!preg_match("/^[a-zA-Z\s]+$/",$lname)){
                ...
                if(filter_var($email, FILTER_VALIDATE_EMAIL)){
                    $verify_query = " select * from customer_login where email = '$email'";
                    ...
                    if($count>0){
                        ...
                        if(strlen($password) >= 8){
                            ...
                            $insertquery = "insert into customer_login (First_name, Last_name, email, password) values('$fname', '$lname', '$email', '$en_pass')";
